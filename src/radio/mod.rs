...
mod tests {
    ...
    /// End-to-end over the exact path `main` uses: the factory builds two SDR
    /// links, a codec-encoded message is modulated, flown over the channel, and
    /// decoded identically on the far node.
    #[test]
    fn message_flows_through_the_sdr_factory() {
        ...
        alice.send(&codec::encode(&sent)).unwrap();

        let mut received = None;
        for _ in 0..400 {
            if let Some(frame) = bob.recv().unwrap() {
                received = Some(codec::decode(&frame).unwrap());
                break;
            }
