...
//! node from one tick loop, so `Medium` is shared via `Rc<RefCell<_>>` rather
//! than made `Send`.
...
impl Medium {
    ...
    /// call) — mirrors `UdpChannel`'s one-burst-per-`receive` semantics.
    ...
    /// other — e.g. for a caller that wants to show which nodes can hear which.
...
impl Channel for SimChannel {
    fn transmit(&mut self, samples: &[Iq]) -> Result<(), TransportError> {
        self.medium.borrow_mut().transmit(self.id, samples);
        Ok(())
    }

    fn receive(&mut self) -> Result<Option<Vec<Iq>>, TransportError> {
        Ok(self.medium.borrow_mut().take_burst(self.id))
    }
