...
use kaem_node::{Author, Contact, Message};

use crate::tui::theme;

/// `Message.timestamp` is virtual epoch-ms (`kaem_node::Time`); the widget
/// boundary is where it becomes a calendar `DateTime` for display/grouping.
fn as_datetime(message: &Message) -> DateTime<Utc> {
    DateTime::from_timestamp_millis(message.timestamp as i64).unwrap_or_default()
}
...
impl MessagePanel<'_> {
    fn build_lines(&self, sender_w: usize, text_w: usize, full_w: usize) -> Vec<Line<'static>> {
        ...
        for message in &self.contact.history {
            let when = as_datetime(message);
            let cur_date = when.date_naive();
            ...
            let new_block = day_changed
                || prev.as_ref().is_none_or(|p| {
                    p.message.author != message.author
                        || (when - p.when).num_seconds() > GROUP_WINDOW_SECS
                });
            ...
            for (i, segment) in wrap(&message.body, text_w).into_iter().enumerate() {
                ...
                let time = if first {
                    Span::styled(format!("{:<5}", when.format("%H:%M")), theme::meta())
                } else {
                    Span::raw(" ".repeat(5))
                };
                let name = if first {
